    ...
    $(document).ready(function(){
         ...
         var loop_c = focus_dup_id.match(/[\d\.]+/g);
                if (loop_c != null){
                    ...
                    var get_val_menuitem=$("#Md_Menu_Definition_"+loop_count+"_MMO_ID").val();
                    if(get_val_menuitem!=''){
                     $(".table").text("You Are Also Select Menu Item On This Row !");
                     $("#sub_menu_autocomplete").hide();
                       return false;                       
                    }
                }       
        $(".select_item").dblclick(function(){
            ...
            $("#Md_Menu_Definition_"+loop_count+"_SUBMENU_MMD_ID_HIDDEN").val(item_prompt);
            ...
            //$("#Md_Menu_Definition_"+loop_count+"_MENU_PROMPT").val(item_prompt);
            $("#Md_Menu_Definition_"+loop_count+"_SUBMENU_MMD_ID").val(item_id);
...
<div style="width: 100%; height: 30px; float: left">
    <?php 
            ...
            'id' => 'sub_menu_autocomplete',
            'name' => 'nombre',
            'source'=>$this->createUrl('md_menu_definition/autocompleteTest_For_Submenu'),
            //'value' =>  $model->nombre,
            'options' => array(
                'showAnim' => 'fold',
                ...
                'select'=>'js:function(event, ui)   {
                                            ...
                                            $("#Md_Menu_Definition_"+loop_count+"_SUBMENU_MMD_ID_HIDDEN").val(ui.item.label); 
                                            ...
                                            $("#Md_Menu_Definition_"+loop_count+"_SUBMENU_MMD_ID").val(ui.item.id);
                                            $("#mydialog").dialog("close");
                                                     }'
            ),
            'htmlOptions' => array(
...
// You can work with the variable passed to the view file
        ...
        $criteria->select='ID,MENU_PROMPT,DESCRIPTION';
        $criteria->addCondition('MMD_ID IS NULL');
        $criteria->order = 'ID ASC';
        
        $items = Md_Menu_Definition::model()->findAll($criteria);
    ...
    <tr class="select_item" style="cursor: pointer">
        ...
        <td><?php echo $val['MENU_PROMPT'];?></td>
