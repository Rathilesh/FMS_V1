	...
	init:function(setting){
		...
		jQuery(function($){ //on document.ready
			...
			if (setting.revealfx[0]=="slide"){ //create outer DIV that will house all the fieldset.sectionwrap elements
				$sectionswrapper=$('<div class="tot_out_class" style="position:relative;overflow:hidden;"></div>').insertBefore($sections.eq(0)) //add DIV above the first fieldset.sectionwrap element
				$sectionswrapper_inner=$('<div style="position:absolute;width:96%;left:0;top:0;"></div>') //create inner DIV of $sectionswrapper that will scroll to reveal a fieldset element
			}
			...
			$sections.each(function(i){ //loop through $sections again
				...
				if (setting.revealfx[0]=="slide"){
					$section.data('page', i).css({position:'absolute',width:'96%', top:0, left:maxfieldsetwidth*i}).appendTo($sectionswrapper_inner) //set fieldset position to "absolute" and move it to inside sectionswrapper_inner DIV
				}
				...
				var $thestep=$('<div class="step disabledstep" />').data('section', i).html('Step '+(i+1)+'<span style="font-size: 19px; padding-left: 5px; text-shadow: 0px 3px 2px rgb(15, 78, 28);color: rgb(192, 248, 237);"><img src="./../../assets/form_wizard/arrow-right-icon.png" width="18"></span><div class="smalltext">'+$section.find('legend:eq(0)').text()+'</div>').appendTo($stepsguide)
