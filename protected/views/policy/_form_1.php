...
//$.ajax({
//                        type: "POST",
//                        url:  "<?php //echo $this->createUrl('Policy/Create') ?>",
//                        data: "theID=DESC",
...
//                    })
url="<?php echo $this->createUrl('Policy/Create') ?>"+"?theID=DESC";
 $.ajax({
                "url":url,
                //"cache":false,
                "success": function(html){
                        jQuery("#mainform").html(html);
                }
        });
}
            ...
            {                
               ...
               for(var j=1;j<=count;j++)
               {
                    ...
                    if(policyvalue!="" || policydescription!="")
                    {
                        ...
                        if(policyvalue=="")
                        {
                            GetDynamicAlert("20049",i);
                            $("#POLICY_VALUE-\\["+ j +"\\:"+ i +"\\]").focus();
                            return false;
                        }
                        if(policydescription=="")
                        {
                            GetDynamicAlert("20050",i);
...
//", CClientScript::POS_READY);
    ...
    if(isset($_POST['username']))
    {
        ...
        $type = $_GET['theID'];
           ...
           $type="ASC" ;
        //echo $type;
        $getPolicyMaster=Policy::model()->findAll(array('order'=>'UPPER(SHORT_NAME) '.$type));        
                                      ...
                                      for($j=1;$j<=$detailscount; $j++){
                                          ...
                                          if(isset($_POST['policydetailsid-'.$j.':'.$i])<>"")
                                          {
                                              ...
                                              if($policydetailsid[$j]=="")
                                                  ...
                                                  $detailsdeletestyle="display:block;float:right;margin: -1px 5px 0px 0px;cursor:pointer";
                                          }
                ...
                <tr>
                    <td class="btn_save">
                        <?php $this->widget('bootstrap.widgets.TbButton', array('buttonType'=>"submit",'size'=>'small', 'label'=>$model->isNewRecord ? $Getlabelname->Get_Labelname_Fnc("103") : $Getlabelname->Get_Labelname_Fnc("2021"),'id'=>"btn_Save",'icon'=>'icon-thumbs-up','htmlOptions'=>array('onclick'=>'js:return SaveAllPolicy();'))); ?>            
                         <?php $this->widget('bootstrap.widgets.TbButton', array('buttonType'=>"reset",'size'=>'small','icon'=>'icon-refresh', 'label'=>"Reset")); ?>
    ...
    $this->endWidget(); 
    ?>
<!--    <button id="mytest"  value="test" onclick="mytest()">test</button>-->
</div>