...
//				};
               ...
               // ###### EDITED BY RATHILESH ###### CHANGE NAME AND ID FOR CLONED ROW ########### //
                //get table name
                var table_name=$("#get_table_name").val();
               // alert(table_name);
                $(clone).find('[id]').each(function(){
                        var newid = table_name+"_"+ (counter +1)+"_"+$(this).attr('class') ;
                        funcBeforeNewId.call($(this));
                        $(this).attr('id', newid);
                        funcAfterNewId.call($(this));
                });
                $(clone).find('[name]').each(function(){
                    var newid = table_name+"["+ (counter +1)+"]["+$(this).attr('class')+"]" ;
                    funcBeforeNewId.call($(this));
                    $(this).attr('name', newid);
                    funcAfterNewId.call($(this));
                });
                $("#TotRowCount").val(counter +1);
